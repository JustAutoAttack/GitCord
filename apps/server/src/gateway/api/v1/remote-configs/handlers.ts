...
export const getByGuildAndRemoteHandler: RouteHandler<
	typeof getByGuildAndRemoteRoute
> = async (ctx) => {
...
export const updateHandler: RouteHandler<typeof updateRoute> = async (ctx) => {
	...
	httpLogger.info(`API Success: Updated remote configuration [ID: ${id}]`);
...
export const deleteHandler: RouteHandler<typeof deleteRoute> = async (ctx) => {
	...
	httpLogger.info(`API Success: Deleted remote configuration [ID: ${id}]`);
	return ctx.json(
		{
			success: true,
